...
function loadDrawerState() {
  try {
    ...
    if (savedState) {
      mobileDrawerState = JSON.parse(savedState);
      console.log('从本地存储加载抽屉状态:', mobileDrawerState);
    }
  } catch (error) {
...
function saveDrawerState() {
  try {
    ...
    console.log('保存抽屉状态到本地存储:', mobileDrawerState);
  } catch (error) {
...
function initMobileDrawer() {
  ...
  // 创建滚动锁定指示器
  const scrollLockIndicator = document.createElement('div');
  scrollLockIndicator.className = 'scroll-lock-indicator';
  scrollLockIndicator.innerHTML = '<i class="bi bi-lock"></i>';
  drawer.appendChild(scrollLockIndicator);
